      ...
      />
        <Tooltip title="Add New Pipe">
            <Fab color="primary" aria-label="add" style={{marginLeft: '20px'}} >
              <div >
                <AddIcon onClick={()=> (navigate('/admin/add/pipe/'))}/>
              </div>
            </Fab>
        </Tooltip>
      </>
...
export default function AllPipeTable({headCells, rows, TableName}) {
  ...
  const [pipeDeleteOpen, setPipeDeleteOpne] = React.useState(false);   // States to open pipe delete modal
  const [pipeID, updatePipeID] = useState(0);

  const navigate = useNavigate()
  
  // Navigate to update page after clicking on Edit Button
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell 
                           align="left" 
                           style={{fontFamily: 'sedan', fontSize: '15px'}}>
                            <b>{row.process_curr.name}</b>
                    </TableCell>

                    {/* Status Column */}
                    <TableCell align="left">
                      <button type="button" className={`btn btn-outline-${getStatusColor(row.is_active ? 'Active' : "Inactive")} my-2`}>
                    ...
                    <TableCell align="left">
                        <Badge color="success" >
                            <EditIcon color="" style={{color:'#0e3080'}} onClick={()=> {handleEditButtonClicked(row); }}  />
                            <DeleteIcon style={{color:'#b23344'}} onClick={()=> {handleDeleteButtonClicked(row.id)}} />
                        </Badge>
                    </TableCell>
